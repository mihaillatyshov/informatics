...
begin
  ...
  else if ((ArrM[0] <= ArrN[Length(ArrN) - 1]) and (ArrM[Length(ArrM) - 1] >= ArrN[Length(ArrN) - 1]))
    LastVal := FindThr(ArrM, ArrN, K)
  else if ((ArrN[0] <= ArrM[Length(ArrM) - 1]) and (ArrN[Length(ArrN) - 1] >= ArrM[Length(ArrM) - 1]))
